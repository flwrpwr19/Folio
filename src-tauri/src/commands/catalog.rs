                    ...
                    tauri::async_runtime::spawn(async move {
                        ...
                        let res = tauri::async_runtime::spawn_blocking(move || {
                            ...
                            for path in &event.paths {
                                let is_supported = match path.extension().and_then(|ext| ext.to_str()) {
                                    Some(ext) => {
                                        let ext_lower = ext.to_ascii_lowercase();
                                        matches!(ext_lower.as_str(), "jpg" | "jpeg" | "png" | "webp" | "gif" | "bmp" | "heic" | "heif" | "dng" | "raw" | "cr2" | "nef" | "arw" | "mp4" | "mov" | "mkv" | "avi")
                                    }
                                    None => false,
                                };
                                ...
                                if path.exists() {
                                    if is_supported && path.is_file() {
                                        if let Ok(metadata) = media_core::read_metadata(path) {
                                            let _ = state_for_blocking.cache.upsert_metadata(path, &metadata);
                                            ...
                                            if let Some(index) = &mut *index_lock {
                                                if let Some(existing) = index.items.iter_mut().find(|it| it.path == *path) {
                                                    *existing = item;
                                                } else {
...
#[tauri::command]
pub async fn create_physical_folder(parent_path: String, folder_name: String, state: State<'_, Arc<AppState>>) -> Result<(), String> {
    if folder_name.contains('/') || folder_name.contains('\\') || folder_name.contains("..") {
        return Err("Invalid folder name: traversal segments or path separators are forbidden".to_string());
    }
    
    let parent = std::path::PathBuf::from(&parent_path);
    if !crate::is_path_safe(&parent, &state) {
        return Err("Permission denied: parent directory lies outside safe sandbox boundaries".to_string());
    }
...
pub async fn delete_physical_file(path: String, state: State<'_, Arc<AppState>>) -> Result<(), String> {
    ...
    if !crate::is_path_safe(&p, &state) {
        return Err("Permission denied: path deletion lies outside safe sandbox boundaries".to_string());
    }
