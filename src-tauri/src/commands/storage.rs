...
#[tauri::command]
pub async fn get_storage_diagnostics(state: State<'_, std::sync::Arc<AppState>>) -> Result<StorageDiagnostics, String> {
...
pub async fn purge_cache(state: State<'_, std::sync::Arc<AppState>>) -> Result<(), String> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        // 1. Purge DB tables
        {
            ...
            conn.execute("DELETE FROM image_metadata", []).map_err(|e| e.to_string())?;
            conn.execute("DELETE FROM albums", []).map_err(|e| e.to_string())?;
            conn.execute("DELETE FROM album_images", []).map_err(|e| e.to_string())?;
            conn.execute("DELETE FROM tags", []).map_err(|e| e.to_string())?;
            conn.execute("DELETE FROM image_tags", []).map_err(|e| e.to_string())?;
            conn.execute("DELETE FROM visual_histograms", []).map_err(|e| e.to_string())?;
